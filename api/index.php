...
function updateDossier($id) {
    ...
    if ($httpCode === 400 && preg_match("/Could not find the '([^']+)' column of 'dossiers' in the schema cache/", $response, $matches)) {
        $missingColumn = $matches[1];
        if (isset($data[$missingColumn])) {
            unset($data[$missingColumn]);
            $payload = json_encode($data);
            $response = sendSupabaseRequest($url, 'PATCH', $payload, $httpCode, $curlError);
            logDebug("PATCH dossier retry without missing column", ['id' => $id, 'missingColumn' => $missingColumn, 'httpCode' => $httpCode, 'error' => $curlError, 'response' => $response]);
